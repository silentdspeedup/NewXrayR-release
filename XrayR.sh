...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/silentdspeedup/NewXrayR-release/refs/heads/main/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/silentdspeedup/NewXrayR-release/refs/heads/main/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_menu() {
    echo -e "
  ${green}XrayR 后端管理脚本，${plain}${red}不适用于docker (Docker功能已禁用)${plain}
--- https://github.com/silentdspeedup/NewXrayR ---
  ${green}0.${plain} 修改配置
————————————————
